...
impl<'a> FunctionTranslator<'a> {
    ...
    fn translate_expr(&mut self, expr: Expr) -> Value {
        match expr {
            ...
            Expr::Loop(condition, loop_body) => {
                self.translate_loop(*condition, loop_body)
            }
        }
    }
    ...
    fn translate_loop(&mut self, condition: Option<Expr>, loop_body: Vec<Expr>) -> Value {
        ...
        let condition_value = self.translate_expr(condition.unwrap_or(Expr::Literal(String::from("1"))));
...
fn declare_variables_in_stmt(
    ...
    match *expr {
        ...
        Expr::Loop(ref _condition, ref loop_body) => {
