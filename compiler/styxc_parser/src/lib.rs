...
pub fn parse<'i>(source: &'i String) -> Result<Pairs<'i, Rule>, Box<dyn Error>> {
    match TokenParser::parse(Rule::exprs, source) {
        Ok(r) => Ok(r),
        Err(e) => Err(e.into()),
    }
}
...
mod tests {
    ...
    fn test_ident() {
        // x
        let mut res = TokenParser::parse(Rule::ident, "x").unwrap_or_else(|e| panic!("{}", e));
        ...
        // someFunc_1234
        let mut res =
            TokenParser::parse(Rule::ident, "someFunc_1234").unwrap_or_else(|e| panic!("{}", e));
        let res = res.next().expect("Expected match for rule ident");
    ...
    fn test_int() {
        // 1234
        let mut res = TokenParser::parse(Rule::int, "1234").unwrap_or_else(|e| panic!("{}", e));
        ...
        // -4321
        let mut res = TokenParser::parse(Rule::int, "-4321").unwrap_or_else(|e| panic!("{}", e));
        let res = res.next().expect("Expected match for rule int");
        assert_eq!(res.as_rule(), Rule::int);
        ...
        // 0b1011101
        let mut res =
            TokenParser::parse(Rule::int, "0b1011101").unwrap_or_else(|e| panic!("{}", e));
        let res = res.next().expect("Expected match for rule int");
        assert_eq!(res.as_rule(), Rule::int);
        ...
        // -0d123456890
        let mut res =
            TokenParser::parse(Rule::int, "-0d123456890").unwrap_or_else(|e| panic!("{}", e));
        let res = res.next().expect("Expected match for rule int");
        assert_eq!(res.as_rule(), Rule::int);
        ...
        // 0o1234567
        let mut res =
            TokenParser::parse(Rule::int, "0o1234567").unwrap_or_else(|e| panic!("{}", e));
        let res = res.next().expect("Expected match for rule int");
        assert_eq!(res.as_rule(), Rule::int);
        ...
        // 0xffff
        let mut res = TokenParser::parse(Rule::int, "0xffff").unwrap_or_else(|e| panic!("{}", e));
        let res = res.next().expect("Expected match for rule int");
        assert_eq!(res.as_rule(), Rule::int);
    ...
    fn test_float() {
        // 1234.5
        let mut res = TokenParser::parse(Rule::float, "1234.5").unwrap_or_else(|e| panic!("{}", e));
        ...
        // -543.21
        let mut res =
            TokenParser::parse(Rule::float, "-543.21").unwrap_or_else(|e| panic!("{}", e));
        let res = res.next().expect("Expected match for rule float");
        assert_eq!(res.as_rule(), Rule::float);
        ...
        // 23e7
        let mut res = TokenParser::parse(Rule::float, "23e7").unwrap_or_else(|e| panic!("{}", e));
        let res = res.next().expect("Expected match for rule float");
        assert_eq!(res.as_rule(), Rule::float);
        ...
        // 32e-72
        let mut res = TokenParser::parse(Rule::float, "32e-72").unwrap_or_else(|e| panic!("{}", e));
        let res = res.next().expect("Expected match for rule float");
        assert_eq!(res.as_rule(), Rule::float);
    ...
    fn test_char() {
        // 'a'
        let mut res = TokenParser::parse(Rule::char, "'a'").unwrap_or_else(|e| panic!("{}", e));
        ...
        // '\n'
        let mut res = TokenParser::parse(Rule::char, "'\\n'").unwrap_or_else(|e| panic!("{}", e));
        let res = res.next().expect("Expected match for rule char");
        assert_eq!(res.as_rule(), Rule::char);
        ...
        // '\uFF0F'
        let mut res =
            TokenParser::parse(Rule::char, "'\\uFF0F'").unwrap_or_else(|e| panic!("{}", e));
        let res = res.next().expect("Expected match for rule char");
        assert_eq!(res.as_rule(), Rule::char);
    ...
    fn test_string() {
        // "hello world"
        let mut res =
            TokenParser::parse(Rule::string, "\"hello world\"").unwrap_or_else(|e| panic!("{}", e));
        ...
        // "hello, \u60ff"
        let mut res = TokenParser::parse(Rule::string, "\"hello, \\u60ff\"")
            .unwrap_or_else(|e| panic!("{}", e));
        let res = res.next().expect("Expected match for rule string");
        assert_eq!(res.as_rule(), Rule::string);
        assert_eq!(
        ...
        // hello, 🦊
        let mut res =
            TokenParser::parse(Rule::string, "\"hello, 🦊\"").unwrap_or_else(|e| panic!("{}", e));
        let res = res.next().expect("Expected match for rule string");
        assert_eq!(res.as_rule(), Rule::string);
