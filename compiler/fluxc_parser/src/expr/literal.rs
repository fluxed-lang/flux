...
pub(crate) fn literal_str() -> impl Parser<Token, String, Error = Simple<Token>> {
    select! { Token::LiteralStr(x) => x }.labelled("str")
}

/// Parser combinator for literal integers.
pub(crate) fn literal_int() -> impl Parser<Token, i64, Error = Simple<Token>> {
...
pub(crate) fn literal_float() -> impl Parser<Token, f64, Error = Simple<Token>> {
